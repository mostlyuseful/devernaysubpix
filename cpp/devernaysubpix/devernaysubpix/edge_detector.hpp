...
struct PartialImageGradients {
  ...
  PartialImageGradients(cv::Mat const &horz, cv::Mat const &vert)
      : horz(horz), vert(vert) {}
  ...
  /**
   * @brief Evaluates gradient tuple at location of point. Coordinates are
   * rounded by casting to int
   * @param p The point
   * @return Gradient value as CurvePoint with valid:=false
   */
  ...
  /**
   * @brief Computes gradient magnitude at location of point. Coordinates are
   * rounded by casting to int
   * @param p The point
   ...
   * @return The gradient magnitude
   */
  template <typename T> inline T magnitude(int const row, int const col) const {
    T const dx = horz.at<T>(row, col);
    T const dy = vert.at<T>(row, col);
    return hypot(dx, dy);
  }
  ...
  /**
   * @brief Thresholds gradient magnitude, returns !=0 where local_magnitude >
   * min_magnitude
   * @param min_magnitude The lower magnitude threshold. All gradient magnitudes
   * higher than this will be set to non-zero
...
/**
 * @brief The PossibleCurvePoint struct bundles a point and signifies whether it
 * has local extremal gradient magnitude
 */
struct PossibleCurvePoint {
  bool isLocalExtremum;
  CurvePoint point;

  PossibleCurvePoint(bool isLocalExtremum, CurvePoint const &point)
      : isLocalExtremum(isLocalExtremum), point(point) {}
};

/**
 * @brief Tests if point at (row,col) is a local gradient extremum and computes
 * sub-pixel location refinement
 * @param gradients Gradient field used for maximum testing and sub-pixel
 * refinement
 * @param row
 * @param col
 * @return A PossibleCurvePoint instance with isLocalExtremum set to true if
 * refinement was successful, in this case member 'point' is populated with
 * refined coordinates
 */
...
compute_single_edge_point(PartialImageGradients const &gradients, int row,
  ...
  std::function<float(int,int)> mag = [&gradients](int row, int col) -> float {
  ...
  if (theta_x || theta_y) {
    ...
    return PossibleCurvePoint(false, CurvePoint());
  }
}
...
compute_edge_points(PartialImageGradients gradients, cv::Mat mask) {
  ...
  if (gradients.horz.channels() != 1) {
    throw std::runtime_error(
        "Horizontal gradient strength image must have single channel");
  }

  if (gradients.vert.channels() != 1) {
    throw std::runtime_error(
        "Vertical gradient strength image must have single channel");
  }
...
struct NearestNeighborhoodPoints {
  ...
  NearestNeighborhoodPoints(bool forward_valid, bool backward_valid,
                            CurvePoint forward, CurvePoint backward)
      : forward_valid(forward_valid), backward_valid(backward_valid),
...
NearestNeighborhoodPoints inline find_nearest_forward_and_backward(
  ...
  for (std::vector<CurvePoint>::const_iterator p = neighborhood.cbegin();
       p < neighborhood.cend(); ++p) {
    // Gradient values at g(n.x, n.y)
    CurvePoint gn(gradients.at<float>(*p));
    float distance = reference.distance(*p);
    bool const angle_lower_than_90_degrees = dot(ge, gn) > 0;
    if (!angle_lower_than_90_degrees) {
      continue;
    }

    float const angle_perp = dot((*p) - reference, perpendicular_vec(ge));
...
NeighborhoodBitmap inline precompute_neighborhood_bitmap(
  ...
  for (std::vector<CurvePoint>::const_iterator e = edges.cbegin();
       e < edges.cend(); ++e) {
    bitmap.set(*e);
  }
  return bitmap;
...
LinkMap inline chain_edge_points(std::vector<CurvePoint> const &edges,
  ...
  for (std::vector<CurvePoint>::const_iterator e = edges.cbegin();
    ...
    if (nearest.backward_valid) {
      CurvePoint const &b = nearest.backward;
      // L10
      if (!links.hasLeft(b) ||
...
typedef std::vector<CurvePoint> Chain;
...
Chains inline thresholds_with_hysteresis(std::vector<CurvePoint> &edges,
  ...
  // chaining
  for (std::vector<CurvePoint>::iterator e = edges.begin();
       e < edges.end(); ++e) {
    e->valid = false;
  }
    ...
    if (!(e->valid) && grads.magnitude<float>(*e) >= high_threshold) {
      ...
      while (links.hasLeft(f) && !(links.getByLeft(f).second.valid) &&
             (grads.magnitude<float>(links.getByLeft(f).second) >=
              low_threshold)) {
        CurvePoint n = links.getByLeft(f).second;
        n.valid = true;
        links.replace(f, n);
        f = n;
        forward.push_back(f);
      ...
      while (links.hasRight(b) && !(links.getByRight(b).first.valid) &&
             (grads.magnitude<float>(links.getByRight(b).first) >=
              low_threshold)) {
        CurvePoint n = links.getByRight(b).first;
        n.valid = true;
        links.replace(n, b);
        b = n;
